...
class UserVeridator {
  ...
  public function loginVerification($username, $password){
    $result = Database::get()->execute('SELECT * FROM member WHERE active = "Yes" AND username = :username', array(':username' => $username));
  ...
  public function isUsernameDuplicate($username){
    $result = Database::get()->execute('SELECT username FROM member WHERE username = :username', array(':username' => $username));
  ...
  public function isEmailDuplicate($email){
    $result = Database::get()->execute('SELECT email FROM member WHERE email = :email', array(':email' => $email));
  ...
  public function isReady2Active($id, $active){
    $result = Database::get()->execute('SELECT username FROM member WHERE id = :id AND active = :active', array(':id' => $id, ':active' => $active));
    if(isset($result[0]['username']) and !empty($result[0]['username'])){
      return true;
    }
    else{
